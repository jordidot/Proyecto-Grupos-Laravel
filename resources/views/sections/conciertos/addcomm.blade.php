...
@section('content')
    <div class="container-lg container-sm  mt-4">
        <div class="container py-3 rounded-top  bg-dark">
            <h3 class="text-white px-2">{{__('web.add_comment_button')}}</h3>
        </div>
        <div class="container h-50 shadow py-3 rounded">
            ...
            <form action="{{route('savecomm')}}" method="POST" class="px-2">
                ...
                <div class="mb-4">
                    <label class="form-label">{{__('web.add_comment_button')}}</label>
                    <textarea name="addComm" class="form-control" rows="5"></textarea>
                </div>

               <div class="mb-5">
                    <button type="submit" class="btn btn-outline-primary px-3">{{__('web.add_comment_button')}}</button>
               </div>
            </form>
        </div>
    </div>
